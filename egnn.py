...
class EGNN(nn.Module):
	def __init__(self, in_node_nf, hidden_nf, out_node_nf, in_edge_nf, device='cuda', act_fn=nn.SiLU(), n_layers=4,
		...
		in_edge_nf: number of features for teh edge features
		device : cpu or cuda
		act_fn: non-linearity
		n_layers: number of layers for EGNN
		residual: use residual connections, recommended
		attention: whether to use attention or not
		normalize: normalizes coordinates messages
		tanh: sets a tanh activation function at the output 
		'''
	...
	def forward(self, h, x, edges, edge_attr):
		h = self.embedding_in(h).to(self.device)
		for i in range(0, self.n_layers):
			h, x, _ = self._modules['gcl_%d'%i](h, edges, x, edge_attr=edge_attr)
		h = self.embedding_out(h)
		return h.to(self.device), x.to(self.device)

...
lagtimes = np.arange(1,201,2, dtype=np.int32)timescales = []
...
# problem here
np.savez(args.save_folder+'/transformed.npz', probs[0])

if args.return_emb:
	np.savez(args.save_folder+'/embeddings.npz', total_emb[0])
if args.return_attn:
	np.savez(args.save_folder+'/total_attn.npz', total_attn[0])
...
np.save(args.save_folder+'/ITS.npy', np.array(timescales))

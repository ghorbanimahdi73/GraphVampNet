...
class EGNN(nn.Module):
	def __init__(self, in_node_nf, hidden_nf, out_node_nf, in_edge_nf=0, device='cpu', act_fn=nn.SiLU(),
		...
		residual: residual connections (better be used)
		attention: whether to use attentoin or not
		normalize: normalizes the coordinates messages such that:
			it may help in hte stability or generalizagiton
		tanh: tanh activation at the output. It bounds the output of 
			phi_x(m_ij) which definitely improves the stability but
			may decrease in accuracy
		'''
...
def unsorted_segment_sum(data, segment_ids, num_segments):
	...
	result = data.new_full(result.shape, 0)
